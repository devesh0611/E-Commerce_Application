    ...
    return (
        <div className="bg-slate-200 p-2 flex gap-4 rounded border-2 border-slate-300">
            <div className="bg-white p-3 rounded overflow-hidden">
                <img src={image} className="h-30 w-40 object-cover"></img>
            </div>
            <div className="flex flex-col gap-1 w-full">
                <div className="flex justify-between">
                <h3 className='font-semibold text-slate-600 capitalize text-lg md:text-xl mt-4'>{name}</h3>
                ...
                <button className="bg-slate-300 py-1 mt-2 rounded hover:bg-slate-400 px-1 text-2xl" onClick={()=>dispatch(increaseQty(id))}><TbPlus/></button>
                <p className="font-semibold p-1">{qty}</p>
                <button className="bg-slate-300 py-1 mt-2 rounded hover:bg-slate-400 px-1 text-2xl" onClick={()=>dispatch(decreaseQty(id))} ><TbMinus/></button>
                </div>
